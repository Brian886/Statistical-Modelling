...
summary(model1)
#wage = B0 +B1*exper + e
#wage_hat <- 5.37331 + 0.03*exper (estimated y, so we wont add error)
#B0: exper = 0 if there is no experience of the employee, the expected hourly earning is 5.37 dollars
#B1: 0.03 plus one year in experience results 0.03 dollar increase 
#in the hourly earning on average
...
#H0: B(exper) = 0 (no significant effect on X and Y)
...
#we reject the H0, the model is relevant, there is at least one significant variable
...
#hourly average earning of female is less by 1.74 dollars compared to a male worker 
...
#Calculare the adjusted R squard in the second model
...
#it is righ sided test
#F: 58.403
#pvalue is almost 0
#we reject the H0, at least one of these variables has significant effect on wages
...
#Calculate the chi2 test and the p-value manually
...
#add the interaction between experience and female dummy and the square of experience 
model3 <- lm(wage ~ educ + exper + tenure + nonwhite + female + married + exper*female+ I(exper^2), data = wages)
summary(model3)
#by changing specification of the model, changing the functional form
#the experience variable became significant

#we use  -0.0041162 for sth sth i dont know (check later)
